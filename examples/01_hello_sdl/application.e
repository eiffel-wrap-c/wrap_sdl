		...
		do
			...
			if {SDL_FUNCTIONS_API}.sdl_init ({SDL_CONSTANT_API}.SDL_INIT_VIDEO) < 0 then
				...
				if window = Void then
					...
					screen_surface := {SDL_VIDEO}.sdl_get_window_surface (window)
					-- Get window surface
					if screen_surface = Void then
						...
						if attached screen_surface.format as l_format then
							if {SDL_SURFACE}.sdl_fill_rect (screen_surface, Void, {SDL_PIXELS_FUNCTIONS_API}.sdl_map_rgb (l_format, 0xFF, 0xFF, 0xFF)) /= 0 then
								print( "SDL Fill rect! SDL_Error: "+ {SDL_ERROR}.sdl_get_error )
								{EXCEPTIONS}.die (1)
							end
						end
							-- Update the surface
						if  {SDL_VIDEO}.sdl_update_window_surface (window) /= 0 then
							print( "SDL update the surface! SDL_Error: "+ {SDL_ERROR}.sdl_get_error )
							{EXCEPTIONS}.die (1)
						end
						{SDL_TIMER_FUNCTIONS_API}.sdl_delay( 5000 )
							-- wait 5 seconds
					end
					{SDL_VIDEO}.sdl_destroy_window (window)
