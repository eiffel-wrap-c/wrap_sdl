...
class SDL_WINDOWS_MESSAGE_HOOK_DISPATCHER

inherit

	EWG_SDL2_CALLBACK_C_GLUE_CODE_FUNCTIONS_API
		export {NONE} all end
	DISPOSABLE
...
feature -- Initialization
	...
	make
		...
		do
			set_sdl_windows_message_hook_object ($Current)
		end

feature -- Access: Routine 

	routine_1: detachable PROCEDURE [TUPLE [a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER]] 
			--Eiffel routine to be call on callback.

	routine_2: detachable PROCEDURE [TUPLE [a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER]] 
			--Eiffel routine to be call on callback.

	routine_3: detachable PROCEDURE [TUPLE [a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER]] 
			--Eiffel routine to be call on callback.
...
feature -- Access: Dispatcher
	...
	c_dispatcher_1: POINTER
		...
		do
			Result := get_sdl_windows_message_hook_stub_1
		end
	...
	c_dispatcher_2: POINTER
		...
		do
			Result := get_sdl_windows_message_hook_stub_2
		end
	...
	c_dispatcher_3: POINTER
		...
		do
			Result := get_sdl_windows_message_hook_stub_3
		end

feature -- Access: Callback

	on_callback_1 (a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER)  
			-- Callback target.
		do
			if attached routine_1 as l_routine then 
				l_routine (a_userdata, a_hwnd, a_message, a_wparam, a_lparam)
			end
		end

	on_callback_2 (a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER)  
			-- Callback target.
		do
	...
	on_callback_3 (a_userdata: POINTER; a_hwnd: POINTER; a_message: INTEGER; a_wparam: INTEGER; a_lparam: INTEGER)  
			-- Callback target.
		do
...
feature -- Register: Callbacks
	...
	register_callback_1 (a_routine: like routine_1)
		...
		do
			routine_1 := a_routine
			set_sdl_windows_message_hook_entry_1 ($on_callback_1)
		ensure
	...
	register_callback_2 (a_routine: like routine_1)
		...
		do
			routine_2 := a_routine
			set_sdl_windows_message_hook_entry_2 ($on_callback_2)
		ensure
	...
	register_callback_3 (a_routine: like routine_1)
		...
		do
			routine_3 := a_routine
			set_sdl_windows_message_hook_entry_3 ($on_callback_3)
		ensure
...
feature {NONE} -- Implementation
	...
	dispose
		...
		do
			...
			set_sdl_windows_message_hook_object (default_pointer)
		end
